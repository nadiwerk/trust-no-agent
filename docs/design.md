...
- `load_skills` failed to resolve `root-cause` in one run (registry staleness, see Design notes #2 below) — the inline fallback is what makes the framework robust to this.
