...
| §3 proposal handoff (model proposes next step by name; user-invoked chain never dead-ends) | Session 2026-09-06 user decision: user-invoked skills stay gated by approval, but a chain that stalls because the user doesn't know the next skill is the user-trigger trap again (check #18's inversion, at chain level) | global | Delete the §3 bullet + eval check #19 + this register row; ledger keeps the record | 2026-09-06 |
