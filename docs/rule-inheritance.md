...
|---|---|---|---|---|
| §6 chat receipt (4-block: Verdict/Evidence/Open/Next) | Session 2026-09-04: user concern chat verifiable drifts without a fixed card shape | global | Delete the 2 router lines + `docs/chat-receipt.md`; ledger keeps the record | 2026-09-04 |
...
| §6 verify the artifact the user reads, never a proxy (live check for rendered claims, `git show` for commit claims) | Session 2026-09-09: 3× false "done" — pushed-commit content verified via `git show` while the user read the GitHub Pages site (site/index.html carries its own install text); recurrence within one session, same wrong-target mechanism each time; user: "bagi saya fatal kalau tidak ada receipt" | global | Delete the §6 router bullet + eval check #21 + this register row; ledger and lessons.md keep the record | 2026-09-09 |
| §6 rendered-output claims need rendered evidence (screenshot/browser of the render), never a string match on the deployed source | Session 2026-09-08/09 site iteration: every site receipt verified a live CSS string while the acceptance target was the rendered page — check #21's wrong-target mechanism in a new form; 9 commits / 3 reverts on one block, all receipts green. Mechanical check: eval #23 + ledger-audit M2 | global | Delete the §6 router bullet + eval check #23 + ledger-audit M2 + this register row; ledger and lessons.md keep the record | 2026-09-09 |
| §6 owner visual feedback is a context to confirm, not an order to execute (restate as a `Context confirmed:` line before the fix) | Session 2026-09-08/09 green-border saga: owner screenshots treated as execution orders — mobile-targeted fixes for a desktop screenshot, sibling blocks restyled unasked; Iron Law 1 has no mechanical gate and initiative-based confirmations fail silently (evals 0/3). Mechanical check: eval #24 + ledger-audit M3 | global | Delete the §6 router bullet + eval check #24 + ledger-audit M3 + this register row; ledger and lessons.md keep the record | 2026-09-09 |
| §2 `Loaded:` audit-trail gap is mechanically detectable (ledger-audit M1 flags domain-relevant entries without a `Loaded:` line, surfaced by doctor C7) | Session 2026-09-09 evaluation: 47 `Loaded: receipts` entries, 0 for root-cause/expect-fail despite domain-relevant work — the §2 audit-trail rule existed but compliance was uncountable; components relying on runtime initiative fail silently, written artifacts work. Mechanical check: ledger-audit.mjs (tested fail-first by ledger-audit.test.mjs) + eval #22 | global | Delete eval check #22 + ledger-audit M1 + doctor C7 + this register row; ledger and lessons.md keep the record | 2026-09-09 |
| §6 `Next: none` only when the whole thread is closed (every close carries a concrete forward arrow) | Session 2026-09-23 owner feedback: closes ending "none pending" mid-chain left the owner without direction ("sebagai user kadang tidak tau harus apa"); ledger drift confirmed. Mechanical check: ledger-audit M7 + test K | global | Delete the §6 bullet + the chat-receipt rule 4 sentence + ledger-audit M7 + test K + this register row; ledger keeps the record | 2026-09-23 |
| §6 chat receipt — delivery: the receipt is plain chat text in the message body, blocks separated with blank lines + bold labels (archive files supplement, never replace) | Session 2026-09-23, twice in one session at the render layer: (1) owner screenshot + sign-off ("ya setuju, file md tidak masalah tapi yang paling penting buat user adalah isi receipt langsung muncul di chat") — 5/5 receipts inside code fences became clipped file cards; (2) owner ("saya tidak terbaca yangmana yang done, evidence, open, next. Pisahkan atau kasih spasi") — tight lines collapsed into one merged paragraph | global | Delete the chat-receipt.md Delivery + Blocks-are-separated paragraphs + this register row; lesson + ledger keep the record | 2026-09-23 |
