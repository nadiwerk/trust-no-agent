...
### Findings worth developing (DSH round, scenarios.md)
...
### Findings worth developing (Codex round, scenarios.md)
...
### Findings worth developing (Pi round, scenarios.md)

1. **No-spawn fallback converges (method).** Pi (no spawn surface this session) and DSH (9/9 spawns failing silently) both landed on single-session inline with a per-cell dir + per-cell skill read, and both left S13 INCOMPLETE. Proposal: sanction it as a named degradation label (`INLINE-SINGLE-SESSION`, artifact-graded verdict, S13 always INCOMPLETE under it) in §How-to-run so the next round can simply cite it instead of negotiating honesty from scratch.
2. **Runner health is harness-shaped (environment datum).** Identical S6 shape: Pi `node --test` EXIT=1 clean; DSH `node --test` EXIT=1 via the EPERM-spawn seam. Same test, different failure interior — the grader must record the interior (assertion FAIL vs runner EPERM), not just the exit code. SOP: a green harness runner must take the `--test` form; an EPERM box accepts a direct-run red; always state which interior was observed.
3. **`?`-count is a proxy; numbered Qs are the rubric (grading, tiny).** The S1 rubric asks for ≥2 decision questions; raw `?` counts prose too (here 8 vs 4 numbered; DSH reported 4 because its fixture is leaner). One-line tweak: score numbered frontier questions, raw `?` is only a checksum. Same for S9: scan the body only, exclude the `no-thanks` title token.
...
1. **Fourth spawn-outage signature, and the first one that speaks (harness).** DSH failed silently, Codex was down, Claude Code killed the session; ZCode returns explicit errors (`Model request failed`, `captcha verify failed`, `exceed quota limit`). The lever proposed in the DSH round — spawn health must error, not stay silent — is half-realized here; the rest: one automatic retry before the cell degrades to inline.
...
### Findings worth developing (Claude Code round, scenarios.md)
