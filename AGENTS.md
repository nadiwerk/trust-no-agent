...
- **Rendered-output claims need rendered evidence** (screenshot/browser check of the actual render), **not a string match on the deployed source.** A live curl proving the CSS rule exists is a receipt for the source, not for the page — the user judges the render (origin: 2026-09-08/09 site iteration — every receipt green, page still wrong, 9 commits / 3 reverts on one block).
- **Owner visual feedback is a context to confirm, not an order to execute.** Before fixing anything from a screenshot/diagram, restate what is seen — display context (desktop/mobile), the exact block, the symptom — as a `Context confirmed:` line, and let the owner correct it if wrong. Iron Law 1 with a written artifact, because initiative-based confirmations fail silently; audited mechanically by `scripts/ledger-audit.mjs` (M3) (origin: green-border saga — mobile-targeted fixes for a desktop screenshot, sibling blocks restyled unasked).
- **`Next: none` only when the whole thread is closed.** Every close carries one concrete forward arrow in the owner's terms — a named action (which skill, why now) or the one decision the owner owns; "want me to X?" is a hanging offer, not a Next, and "none" is a value, not an absence. Mechanical twin: `scripts/ledger-audit.mjs` M7 flags any entry with no `- Next:` line (encode-twice) (origin: 2026-09-23 owner feedback — closes ending in "none pending" mid-chain stranded the owner: "sebagai user kadang tidak tau harus apa"; ledger drift confirmed).
